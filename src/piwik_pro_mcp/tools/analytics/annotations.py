...
def register_analytics_tools(mcp: FastMCP) -> None:
    ...
    def analytics_annotations_create(
        ...
        return client.analytics.create_user_annotation(
            app_id=app_id, content=content, date=date, visibility=visibility
        )
    ...
    def analytics_annotations_get(annotation_id: str, app_id: str) -> Dict[str, Any]:
        """
        Get a user annotation by ID.
        """
        client = create_piwik_client()
        return client.analytics.get_user_annotation(annotation_id=annotation_id, app_id=app_id)

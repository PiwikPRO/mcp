...
OPERATION_PENDING_STATES = frozenset({"created", "started"})


def _extract_operation_state(operation_response: dict[str, Any]) -> str | None:
    data = operation_response.get("data")
...
def _extract_operation_type(operation_response: dict[str, Any]) -> str | None:
    data = operation_response.get("data")
    if not isinstance(data, dict):
        return None
    attributes = data.get("attributes")
...
class TagManagerAPI:
    ...
    def list_tags(
        ...
        if filters is not None:
            for filter_type, value in filters.model_dump(exclude_none=True).items():
    ...
    def create_trigger(
        ...
        **kwargs,
        ...
        attributes_payload = attributes.model_dump(exclude_none=True)
    ...
    def create_draft_snapshot(self, app_id: str) -> dict[str, Any] | None:
        """
        ...
        Raises:
            ...
            PiwikProAPIError: If the request fails
        """
        return self.client.post(f"/api/tag/v1/{app_id}/versions/draft/snapshot")
    ...
    def wait_for_operation(
        ...
        *,
        ...
        Args:
            app_id: App UUID
            operation_id: Operation UUID
            poll_interval_seconds: Delay between state checks
            timeout_seconds: Maximum time to wait for completion

        Returns:
            Final operation response when state is ``completed``
        ...
        while True:
            response = self.get_operation(app_id, operation_id)
            if not isinstance(response, dict):
                raise RuntimeError(
                    f"Unexpected response type while waiting for operation {operation_id}: {type(response).__name__}"
                )

            state = _extract_operation_state(response)
            if state == "completed":
                return response
            if state == "failed":
                operation_type = _extract_operation_type(response)
                details = f" (type: {operation_type})" if operation_type else ""
                raise RuntimeError(f"Operation {operation_id} failed{details}")
            if state not in OPERATION_PENDING_STATES:
                raise RuntimeError(
                    f"Operation {operation_id} has unexpected state '{state}' while waiting for completion"
                )

            remaining = deadline - time.monotonic()
            if remaining <= 0:
                raise RuntimeError(
                    f"Timed out after {timeout_seconds:.0f}s waiting for operation {operation_id} (last state: {state})"
                )
